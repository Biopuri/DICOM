...
class Data(metaclass=DataMeta):
    ...
    def pack_and_dump(cls, value: Any) -> Tuple[bytes, _dump.Dump]:
        ...
        pieces: List[bytes] = []
        try:
            cls.__pack__(value, pieces, dump.add_record(fmt=cls.__name__))
        except Exception as exc:
    ...
    def ipack_and_dump(self) -> Tuple[bytes, _dump.Dump]:
        ...
        pieces: List[bytes] = []
        try:
            self.__pack__(self, pieces, dmp.add_record(fmt=type(self).__name__))
        except Exception as exc:
    ...
    def unpack_and_dump(cls, buffer: bytes) -> Tuple[Any, _dump.Dump]:
        ...
        dmp = _dump.Dump()
        try:
            item, offset = cls.__unpack__(buffer, 0, dmp.add_record(fmt=cls.__name__))
